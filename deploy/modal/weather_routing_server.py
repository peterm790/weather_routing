...
def get_route(
    ...
    if new_times.size == 0 or new_times[-1] != time_vals[-1]:
        new_times = np.concatenate(
            [new_times, np.array([time_vals[-1]], dtype=time_vals.dtype)]
        )
    ...
    ds_lsm = ds_lsm.sel(latitude=slice(max([lat1, lat2]), min([lat1, lat2]))).sel(
    ...
    def progress_callback(step, dist_wp, isochrones, pass_idx=None, emit_prelim=False):
        ...
        if emit_prelim:
            try:
                prelim_route = weatherrouter.get_fastest_route(
                    stats=False, use_optimized=True
                )
                if isinstance(prelim_route, list):
                    ...
                    if "time" in prelim_route_copy.columns:
                        prelim_route_copy["time"] = prelim_route_copy["time"].astype(
                            str
                        )
    ...
    def generate():
        ...
        if optimise_max_passes > 0:
            ...
            optimize_thread = threading.Thread(
                target=weatherrouter.optimize, args=(initial_route, initial_isochrones)
            )
        ...
        route_df = weatherrouter.get_fastest_route(
            use_optimized=(optimise_max_passes > 0)
        )
