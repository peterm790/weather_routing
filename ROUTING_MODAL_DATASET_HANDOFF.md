...
Clients and tools must send `provider` first, then `dataset_id`:
...
- Missing or blank `provider` is a contract error.
- Missing or blank `dataset_id` is a contract error.
- Any provider other than `dynamical` returns a structured `400` validation error.
- Any dataset id other than `gfs` or `aifs` returns a structured `400` validation error.
- Old longer dataset ids are not accepted or translated.
...
- Do not generate client or tool examples using old longer dataset ids.
